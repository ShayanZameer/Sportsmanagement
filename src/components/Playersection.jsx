...
const Playersection = ({ text, showAddButton, buttonText }) => {
  return (
    <div className='flex flex-col xm:flex-col xmm:flex-col sm:flex-row items-center py-4 px-4 sm:px-6 md:px-8 lg:px-10 xl:px-4 xmm:justify-between w-[1050px] '>
      <div className='flex items-center mb-4 xm:mb-0'>
        <p className=' font-normal font-bebas text-[36px] xm:text-[40px] sm:text-[44px] md:text-[48px] lg:text-[52px] xl:text-[56px]'>{text}</p>
        {showAddButton && (
          <div className='ml-4'>
            <Link
              to="/user-profile"
              ...
              style={{ clipPath: 'polygon(10% 0%, 90% 0%, 100% 50%, 90% 100%, 10% 100%, 0% 50%)' }}
            >
              <span className="mr-2 text-xs sm:text-sm md:text-base">{buttonText}</span>
              <AiOutlinePlus className="text-base sm:text-lg md:text-xl" />
            </Link>
          </div>
        )}
